...
export async function execute(ctx) {
    ...
    const sessionState = normalizeSession(ctx.sessionParams);
    ...
    try {
        ...
        await ctx.onLog("stdout", `[ollama] Completed. Tokens: ${result.usage.totalTokens} (prompt: ${result.usage.promptTokens}, completion: ${result.usage.completionTokens})\n`);
        return {
            exitCode: 0,
            timedOut: false,
            usage: {
...
export async function testEnvironment(ctx) {
    ...
    if (!connectionResult.ok) {
        ...
        if (availableModels.length === 0) {
            ...
            const modelFound = availableModels.some((m) => m === config.model || m.startsWith(`${config.model}:`));
            if (modelFound) {
                checks.push({
